Ammo().then(function(Ammo) {
	...
	class obj3DT {
		...
		destruir () {
			this.removeRigidBody( this._mesh );
			this._mesh = null;
		}
	...
	class paralelepipedoT extends obj3DT {
		constructor( sx, sy, sz, pos, material, massa ) {
			var qMesh = new THREE.Mesh( new THREE.BoxBufferGeometry( sx, sy, sz, 1, 1, 1 ), material );
			var qShape = new Ammo.btBoxShape( new Ammo.btVector3( sx * 0.5, sy * 0.5, sz * 0.5 ) );
			//qShape.setMargin( margin );
			
			super(qMesh, qShape, pos, massa);
		}
	}
	...
	class lacoT {
		...
		novaCorda(tamanho) {
			...
			this._corda = new cordaT(tamanho, posI, posF);
	...
	function initObjs() {
		...
		teto = new paralelepipedoT(totCubos * tamanhoGrid, 4, 100, 0, pos, material, 100 );
		teto.body.setUserIndex(ID_TETO);
...
//lanca.userData.physicsBody.setLinearVelocity( new Ammo.btVector3( 250, 250, 0 ) );
		...
		// CHAO
		pos.set( 0,-tamanhoGrid,0 );
		chao = new paralelepipedoT(totCubos * tamanhoGrid, 4, 100, 0, pos, material, 100 );
		//chao.body.setUserIndex(ID_CHAO);
